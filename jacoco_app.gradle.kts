...
val unitTestReport = task<JacocoReport>("jacocoStageUnitTestReport") {
    ...
    executionData.setFrom(
        fileTree(project.buildDir) {
            include(
                "outputs/unit_test_code_coverage/stageDebugUnitTest/testStageDebugUnitTest.exec"
            )
        }
    )
}
    ...
    doLast {
        ...
        ant.withGroovyBuilder {
            "report"{
                "executiondata"{
                    ant.withGroovyBuilder {
                        "file"("file" to "${buildDir.path}/outputs/unit_test_code_coverage/stageDebugUnitTest/testStageDebugUnitTest.exec")
                    }
                }

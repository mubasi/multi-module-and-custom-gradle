...
class AdapterSearchMallLocation(
    private val _vm: SearchMallLocationViewModel
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private val data: MutableList<LocationModel> = arrayListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val binding = DataBindingUtil.inflate<ItemParentSearchLocationBinding>(
            LayoutInflater.from(parent.context),
            R.layout.item_parent_search_location,
            parent,
            false
        )
        return when (viewType) {
            LocationModel.CHILD -> ChildViewHolder(
                ...
                DataBindingUtil.inflate(
                    LayoutInflater.from(
                        parent.context
                    ), R.layout.item_child_search_location, parent, false
                )
            )

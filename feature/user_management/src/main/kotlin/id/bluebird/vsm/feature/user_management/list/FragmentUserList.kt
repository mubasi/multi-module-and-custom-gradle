...
class FragmentUserList : Fragment() {
    ...
    private fun gotoCreateUser(id: Long?, uuid: String) {
        val destination =
            FragmentUserListDirections.actionUserListFragmentToCreateUserFragment().apply {
                userId = id ?: 0
                this.uuid = uuid
            }
        findNavController().navigate(destination)
        setFragmentResultListener(FragmentCreateUser.REQUEST_KEY) { _, bundle ->
            bundle.let {
                mUserSettingVM.result(
                    it.getString(FragmentCreateUser.NAME_PARAM, EMPTY_STRING),
                    it.getParcelable(FragmentCreateUser.ACTION_PARAM) ?: ModifyUserAction.Nothing
                )
            }
        }
    }
